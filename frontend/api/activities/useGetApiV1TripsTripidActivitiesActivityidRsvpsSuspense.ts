/**
 * Generated by Kubb (https://kubb.dev/).
 * Do not edit manually.
 */

import fetch from "../client";
import type { RequestConfig, ResponseErrorConfig } from "../client";
import type {
  QueryKey,
  QueryClient,
  UseSuspenseQueryOptions,
  UseSuspenseQueryResult,
} from "@tanstack/react-query";
import type {
  GetApiV1TripsTripidActivitiesActivityidRsvpsQueryResponse,
  GetApiV1TripsTripidActivitiesActivityidRsvpsPathParams,
  GetApiV1TripsTripidActivitiesActivityidRsvpsQueryParams,
  GetApiV1TripsTripidActivitiesActivityidRsvps400,
  GetApiV1TripsTripidActivitiesActivityidRsvps401,
  GetApiV1TripsTripidActivitiesActivityidRsvps403,
  GetApiV1TripsTripidActivitiesActivityidRsvps404,
  GetApiV1TripsTripidActivitiesActivityidRsvps422,
  GetApiV1TripsTripidActivitiesActivityidRsvps500,
} from "../../types/types.gen.ts";
import { queryOptions, useSuspenseQuery } from "@tanstack/react-query";

export const getApiV1TripsTripidActivitiesActivityidRsvpsSuspenseQueryKey = (
  tripID: GetApiV1TripsTripidActivitiesActivityidRsvpsPathParams["tripID"],
  activityID: GetApiV1TripsTripidActivitiesActivityidRsvpsPathParams["activityID"],
  params?: GetApiV1TripsTripidActivitiesActivityidRsvpsQueryParams,
) =>
  [
    {
      url: "/api/v1/trips/:tripID/activities/:activityID/rsvps",
      params: { tripID: tripID, activityID: activityID },
    },
    ...(params ? [params] : []),
  ] as const;

export type GetApiV1TripsTripidActivitiesActivityidRsvpsSuspenseQueryKey =
  ReturnType<
    typeof getApiV1TripsTripidActivitiesActivityidRsvpsSuspenseQueryKey
  >;

/**
 * @description Returns paginated RSVPs for a specific activity within a trip, optionally filtered by RSVP status.
 * @summary Get activity RSVPs
 * {@link /api/v1/trips/:tripID/activities/:activityID/rsvps}
 */
export async function getApiV1TripsTripidActivitiesActivityidRsvpsSuspense(
  tripID: GetApiV1TripsTripidActivitiesActivityidRsvpsPathParams["tripID"],
  activityID: GetApiV1TripsTripidActivitiesActivityidRsvpsPathParams["activityID"],
  params?: GetApiV1TripsTripidActivitiesActivityidRsvpsQueryParams,
  config: Partial<RequestConfig> & { client?: typeof fetch } = {},
) {
  const { client: request = fetch, ...requestConfig } = config;

  const res = await request<
    GetApiV1TripsTripidActivitiesActivityidRsvpsQueryResponse,
    ResponseErrorConfig<
      | GetApiV1TripsTripidActivitiesActivityidRsvps400
      | GetApiV1TripsTripidActivitiesActivityidRsvps401
      | GetApiV1TripsTripidActivitiesActivityidRsvps403
      | GetApiV1TripsTripidActivitiesActivityidRsvps404
      | GetApiV1TripsTripidActivitiesActivityidRsvps422
      | GetApiV1TripsTripidActivitiesActivityidRsvps500
    >,
    unknown
  >({
    method: "GET",
    url: `/api/v1/trips/${tripID}/activities/${activityID}/rsvps`,
    params,
    ...requestConfig,
  });
  return res.data;
}

export function getApiV1TripsTripidActivitiesActivityidRsvpsSuspenseQueryOptions(
  tripID: GetApiV1TripsTripidActivitiesActivityidRsvpsPathParams["tripID"],
  activityID: GetApiV1TripsTripidActivitiesActivityidRsvpsPathParams["activityID"],
  params?: GetApiV1TripsTripidActivitiesActivityidRsvpsQueryParams,
  config: Partial<RequestConfig> & { client?: typeof fetch } = {},
) {
  const queryKey = getApiV1TripsTripidActivitiesActivityidRsvpsSuspenseQueryKey(
    tripID,
    activityID,
    params,
  );
  return queryOptions<
    GetApiV1TripsTripidActivitiesActivityidRsvpsQueryResponse,
    ResponseErrorConfig<
      | GetApiV1TripsTripidActivitiesActivityidRsvps400
      | GetApiV1TripsTripidActivitiesActivityidRsvps401
      | GetApiV1TripsTripidActivitiesActivityidRsvps403
      | GetApiV1TripsTripidActivitiesActivityidRsvps404
      | GetApiV1TripsTripidActivitiesActivityidRsvps422
      | GetApiV1TripsTripidActivitiesActivityidRsvps500
    >,
    GetApiV1TripsTripidActivitiesActivityidRsvpsQueryResponse,
    typeof queryKey
  >({
    enabled: !!(tripID && activityID),
    queryKey,
    queryFn: async ({ signal }) => {
      config.signal = signal;
      return getApiV1TripsTripidActivitiesActivityidRsvpsSuspense(
        tripID,
        activityID,
        params,
        config,
      );
    },
  });
}

/**
 * @description Returns paginated RSVPs for a specific activity within a trip, optionally filtered by RSVP status.
 * @summary Get activity RSVPs
 * {@link /api/v1/trips/:tripID/activities/:activityID/rsvps}
 */
export function useGetApiV1TripsTripidActivitiesActivityidRsvpsSuspense<
  TData = GetApiV1TripsTripidActivitiesActivityidRsvpsQueryResponse,
  TQueryKey extends QueryKey =
    GetApiV1TripsTripidActivitiesActivityidRsvpsSuspenseQueryKey,
>(
  tripID: GetApiV1TripsTripidActivitiesActivityidRsvpsPathParams["tripID"],
  activityID: GetApiV1TripsTripidActivitiesActivityidRsvpsPathParams["activityID"],
  params?: GetApiV1TripsTripidActivitiesActivityidRsvpsQueryParams,
  options: {
    query?: Partial<
      UseSuspenseQueryOptions<
        GetApiV1TripsTripidActivitiesActivityidRsvpsQueryResponse,
        ResponseErrorConfig<
          | GetApiV1TripsTripidActivitiesActivityidRsvps400
          | GetApiV1TripsTripidActivitiesActivityidRsvps401
          | GetApiV1TripsTripidActivitiesActivityidRsvps403
          | GetApiV1TripsTripidActivitiesActivityidRsvps404
          | GetApiV1TripsTripidActivitiesActivityidRsvps422
          | GetApiV1TripsTripidActivitiesActivityidRsvps500
        >,
        TData,
        TQueryKey
      >
    > & { client?: QueryClient };
    client?: Partial<RequestConfig> & { client?: typeof fetch };
  } = {},
) {
  const { query: queryConfig = {}, client: config = {} } = options ?? {};
  const { client: queryClient, ...queryOptions } = queryConfig;
  const queryKey =
    queryOptions?.queryKey ??
    getApiV1TripsTripidActivitiesActivityidRsvpsSuspenseQueryKey(
      tripID,
      activityID,
      params,
    );

  const query = useSuspenseQuery(
    {
      ...getApiV1TripsTripidActivitiesActivityidRsvpsSuspenseQueryOptions(
        tripID,
        activityID,
        params,
        config,
      ),
      queryKey,
      ...queryOptions,
    } as unknown as UseSuspenseQueryOptions,
    queryClient,
  ) as UseSuspenseQueryResult<
    TData,
    ResponseErrorConfig<
      | GetApiV1TripsTripidActivitiesActivityidRsvps400
      | GetApiV1TripsTripidActivitiesActivityidRsvps401
      | GetApiV1TripsTripidActivitiesActivityidRsvps403
      | GetApiV1TripsTripidActivitiesActivityidRsvps404
      | GetApiV1TripsTripidActivitiesActivityidRsvps422
      | GetApiV1TripsTripidActivitiesActivityidRsvps500
    >
  > & { queryKey: TQueryKey };

  query.queryKey = queryKey as TQueryKey;

  return query;
}
