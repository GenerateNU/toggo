...
export type AddHousingManualSheetHandle = {
  open: (prefill?: Partial<ModelsParsedActivityData>) => void;
  close: () => void;
  setLocation: (loc: PendingLocation) => void;
};

type AddHousingManualSheetProps = {
  tripID: string;
  onSaved: (housing: ModelsActivityAPIResponse) => void;
  onClose: () => void;
};
...
export const AddHousingManualSheet = forwardRef<
  AddHousingManualSheetHandle,
  AddHousingManualSheetProps
>(({ tripID, onSaved, onClose }, ref) => {
  ...
  const priceLabel = price != null ? `$${price.toLocaleString()} USD` : null;
  ...
  return (
    <>
      <AddItemManualSheet
        ...
        saveLabel="Save housing option"
        successMessage="Housing option added"
        onSave={handleSave}
        onSaved={onSaved}
        onClose={onClose}
        formRows={
...
AddHousingManualSheet.displayName = "AddHousingManualSheet";
